...
# ESTRATEGIA HÍBRIDA: REGRESIÓN + EVALUACIÓN CLASIFICACIÓN 

def calcular_metricas_clasificacion(y_real, y_pred, umbral_media):
    """
    Convierte la predicción numérica en binaria (Alta Demanda vs Baja Demanda)
    Clase 1 (Positiva): Ventas > Media histórica (Día de pico)
    Clase 0 (Negativa): Ventas <= Media histórica (Día normal)
    """
    # Binarizar
    y_real_bin = (y_real > umbral_media).astype(int)
    y_pred_bin = (y_pred > umbral_media).astype(int)
    
    # Calcular métricas 
    acc = accuracy_score(y_real_bin, y_pred_bin)
    prec = precision_score(y_real_bin, y_pred_bin, zero_division=0)
    rec = recall_score(y_real_bin, y_pred_bin, zero_division=0) # Sensibilidad
    f1 = f1_score(y_real_bin, y_pred_bin, zero_division=0)
    
    # Especificidad (TN / (TN + FP))
    tn, fp, fn, tp = confusion_matrix(y_real_bin, y_pred_bin, labels=[0,1]).ravel()
...
def entrenar_y_optimizar(df_serie, df_clima):
    ...
    # --- CÁLCULO DE MÉTRICAS 
    metrics_tfm = calcular_metricas_clasificacion(y_test, y_pred, umbral_media)
    metrics_tfm['rmse'] = rmse # Añadimos el RMSE también
    ...
    return best_model, metrics_tfm, df_importancia, umbral_media

def main():
    print("INICIANDO ENTRENAMIENTO + EVALUACIÓN")
    ...
    for idx, row in combinaciones.iterrows():
        ...
        # Obtenemos modelo y el diccionario de métricas completo
        modelo, metrics, df_importancia, umbral = entrenar_y_optimizar(df_serie, df_clima)
        
        if modelo:
            modelos_entrenados[clave_modelo] = {
                'model': modelo,
                'metrics': metrics, # Guardamos TODAS las métricas (F1, Accuracy, RMSE)
                'importance': df_importancia,
                'umbral_demanda_alta': umbral
            }
            # Mostramos RMSE y F1 para verificar que calcula bien
            print(f"✅ RMSE: {metrics['rmse']:.2f} | F1: {metrics['f1_score']:.2f}")
        else:
            print(f"❌ Fallido")
    ...
    print(f" Guardando modelos con métricas en '{ARCHIVO_MODELOS_SALIDA}'...")
